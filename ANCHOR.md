...
## 当前目标
- 当前任务：为 FEATURE_CARDS 增加多交易所 source 能力，并让部分卡片支持欧易（OKX）行情。
...
- 维护轻量、可读的前端代码结构。
- 版本号按 `v主.次.修` 维护；本次属于 feature，升级到 `v1.6.0`。

## 设计笔记
- 将 `fetchPerpTicker` 升级为按 `source` 分流：`binance` 继续走 `fapi/v1/ticker/24hr`，`okx` 走 `api/v5/market/ticker`。
- 在 FEATURE_CARDS 中保留 BTC/XAU 使用 Binance，示例性将 AXS 切换到 OKX（`AXS-USDT-SWAP`）实现“部分支持欧易”。
