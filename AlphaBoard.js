...
// @connect      economia.awesomeapi.com.br
// @license      MIT
// ==/UserScript==
...
(function () {
  ...
  const FEATURE_CARDS = [
    {
      ...
      refreshMs: FEATURE_REFRESH_MS,
    },
    {
      key: 'gold',
      badge: 'XAU',
      name: '黄金 · 实时价',
      source: '数据源 AwesomeAPI',
      fetcher: fetchGoldTicker,
      refreshMs: FEATURE_REFRESH_MS * 2,
    },
  ];
  ...
  function setFeatureState(next){
    const wasExpanded = FEATURE_EXPANDED;
    ...
    if (expandBtn) {
      ...
      if (!nextExpanded && wasExpanded && typeof expandBtn.focus === 'function') {
        expandBtn.focus();
      }
    }
  ...
  if (overlay) {
    overlay.addEventListener('click', (ev)=>{
      if (ev.target === overlay && FEATURE_EXPANDED) setFeatureState(false);
    });
  }
  document.addEventListener('keydown', (ev)=>{
    if (ev.key === 'Escape' && FEATURE_EXPANDED) {
  ...
  async function fetchGoldTicker(){
    try {
      const resp = await gmGetJson('https://economia.awesomeapi.com.br/json/last/XAU-USD');
      const payload = resp?.XAUUSD;
      if (!payload) return null;
      const priceRaw = payload.bid ?? payload.ask ?? payload.high ?? payload.low;
      const changeRaw = payload.varBid;
      const pctRaw = payload.pctChange;
      const price = priceRaw == null ? NaN : parseFloat(priceRaw);
      if (!Number.isFinite(price)) return null;
      const change = changeRaw == null ? NaN : parseFloat(changeRaw);
      const percent = pctRaw == null ? NaN : parseFloat(pctRaw);
  ...
  function startFeaturePollers(){
    FEATURE_CARDS.forEach((item)=>{
      const { key, fetcher, refreshMs } = item || {};
      if (!key || typeof fetcher !== 'function') return;
      if (!featureCardsByKey.has(key)) return;

      let timer = null;
      const interval = typeof refreshMs === 'number' && refreshMs > 0 ? refreshMs : FEATURE_REFRESH_MS;
